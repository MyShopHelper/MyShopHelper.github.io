...
let mainPageWindow = document.querySelector('.main_inf') as HTMLElement;
let innerAboutTextBlock = document.querySelector('.close_about_inf_block') as HTMLElement;
...
if(window.innerWidth <= 700){
    innerAboutTextBlock.style.height = `${window.innerHeight - 61}px`;
    mainPageWindow.style.minHeight = `${window.innerHeight - 61}px`;
...
let getMainAboutBlock = document.querySelector('.modal_about_block') as HTMLElement;

let openMagicCircle = document.querySelector('.modal_about_block') as HTMLElement;
let closeMagicCircle = document.querySelector('.close_about_page') as HTMLElement;
let aboutContentText = document.querySelector('.about_content') as HTMLElement;


for (let i = 0; i < getInnerModalPoints.length; i++) {
    getInnerModalPoints[i].addEventListener('click', ()=>{
        switch(i){
            case 1:
                getMainAboutBlock.classList.remove('hide');
                getModalWindow.classList.remove('show_modal_block');

                setTimeout(()=>{
                    openMagicCircle.classList.add('modal_about_block_fade_in');
                    aboutContentText.classList.remove('about_text_hider');
                    closeMagicCircle.classList.remove('about_text_hider')
                }, 500)
...
closeMagicCircle.addEventListener('click',()=>{
    openMagicCircle.classList.remove('modal_about_block_fade_in');
    aboutContentText.classList.add('about_text_hider');
    getModalWindow.classList.add('show_modal_block');

    setTimeout(()=>getMainAboutBlock.classList.add('hide'),600);
});